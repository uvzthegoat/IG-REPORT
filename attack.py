...
def report_profile_attack(username, proxy):
    ...
    try:
        ...
        print_error("Reporting failed (FacebookRequestsError)")
        return

    if (res.status_code != 200):
        print_error("Reporting failed (STATUS CODE:", res.status_code, ")")
        return

    if ('["_js_datr","' not in res.text):
        print_error("Reporting failed (CookieErrorJSDatr)")
        return
    
    try:
        js_datr = res.text.split('["_js_datr","')[1].split('",')[0]
    except:
        print_error("Reporting failed! (CookieParsingError)")
        return
    ...
    try:
        ...
        print_error("Reporting failed (InstagramRequestsError)")
        return
    ...
    if ("datr" not in res.cookies.get_dict()):
        print_error("Reporting failed (CookieErrorDatr)")
        return
    
    if ('["LSD",[],{"token":"' not in res.text):
        print_error("Reporting failed (CookieErrorLSD)")
        return

    if ('"__spin_r":' not in res.text):
        print_error("Reporting failed (CookieErrorSpinR)")
        return

    if ('"__spin_b":' not in res.text):
        print_error("Reporting failed (CookieErrorSpinB)")
        return

    if ('"__spin_t":' not in res.text):
        print_error("Reporting failed (CookieErrorSpinT)")
        return

    if ('"server_revision":' not in res.text):
        print_error("Reporting failed (CookieErrorRev)")
        return

    if ('"hsi":' not in res.text):
        print_error("Reporting failed (CookieErrorHsi)")
        return
    ...
    try:
        ...
        print_error("Reporting failed (FormRequestsError)")
        return
...
def report_video_attack(video_url, proxy):
    ...
    try:
        ...
        print_error("Reporting failed (FacebookRequestsError)", e)
        return
